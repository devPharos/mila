...
const ClassReport = ({ navigation }) => {
  ...
  useEffect(() => {
    function load() {
      setCurrentFrequency(
        frequency[frequency.findIndex((freq) => freq.period == periodDate)]
      );
    }
            ...
            {group.map((g, index) => {
              return (
                <View key={index} style={{ width: "100%" }}>
                  {index == 0 ? (
                    <Container
                      style={{ width: "100%", marginBottom: 16, height: 100 }}
                    >
                      <View
                        style={{
                          width: "100%",
                      ...
                      >
                        ...
                        >
                          <Text style={{ color: "#222", fontSize: 14 }}>
                            {currentFrequency.totalAbsences || 0}
                          </Text>
                        </View>
                      </View>
                      <View
                      ...
                      >
                        ...
                        >
                          <Text style={{ color: "#222", fontSize: 14 }}>
                            {currentFrequency.percFrequency || 0} %
                          </Text>
                        </View>
                      </View>
                    </Container>
                  ...
                  <Canvas>
                    ...
                    >
                      ...
                      >
                        {capitalizeFirstLetter(g.level)} - Teacher{" "}
                        {capitalizeFirstLetter(g.teacher)}{" "}
                      </Text>
                      <Text style={{ color: "#868686", fontSize: 13 }}>
                        {g.groupID ? g.groupID + " - " : null}
                        {g.name}
                      </Text>
                      <Text style={{ color: "#868686", fontSize: 12 }}>
                        Class SD/ED{" "}
                        {format(parseISO(g.groupStartDate), "MMM, do y")} to{" "}
                        {format(parseISO(g.groupEndDate), "MMM, do y")}
                      </Text>
                      <Text style={{ color: "#868686", fontSize: 12 }}>
                        Student SD/ED{" "}
                        {format(parseISO(g.studentStartDate), "MMM, do y")}{" "}
                        {g.studentEndDate
                          ? " to " +
                            format(parseISO(g.studentEndDate), "MMM, do y")
                          : null}
                      </Text>
                    </View>
                    <View
                        ...
                        >
                          {g.status == "FINISHED" ? (
                            <Text style={{ fontSize: 11 }}>
                              {g.result || "Not Defined"}
                            </Text>
                          ) : (
                            <Text style={{ fontSize: 11 }}>In Progress</Text>
                          )}
                        </CircularProgressBase>
                      </CircularProgressBase>
                      <View>
                        <Text style={{ color: theme.colors.primary }}>
                          {g.givenContentPercentage}% Content given
                        </Text>
                        <Text style={{ color: theme.colors.secondary }}>
                          {g.givenClassPercentage}% Class given
                        </Text>
                      </View>
                    </View>
                    <View
                  ...
                  <ClassesList
                    periodClasses={g.classes.filter(
                      (period) => period.presenceStatus !== ""
                    )}
                    group={g}
                  />
                </View>
