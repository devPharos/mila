...
const ClassReportHeader = ({ setLoading }) => {
  const { period, periodDates, periodDate, setPeriodDate, params } =
  ...
  const animatedStyle = useAnimatedStyle(() => {
    return {
      maxHeight: showPeriodOptions
        ? withTiming(1500, { duration: 1000, easing: Easing.linear })
      ...
      {periodDate ? (
        <>
          ...
          >
            ...
            <TouchableOpacity
              disabled={params.limit_periods_to_students < 1}
          ...
          <Animated.View style={[animatedStyle, styles.container]}>
            <View style={{ paddingVertical: 16 }}>
              {periodDates.map((periodItem, index) => {
                if (
                  periodItem <= format(new Date(), "y-MM") &&
                  periodItem >=
                    format(
                      subMonths(new Date(), params.limit_periods_to_students),
                      "y-MM"
                    )
                ) {
                  return (
                    ...
                    >
                      <TouchableOpacity
                        onPress={() => handlePeriodChange(periodItem)}
                        style={{
                          width: "100%",
                          paddingVertical: 16,
                          flexDirection: "row",
                      ...
                      >
                        ...
                        >
                          {format(parseISO(periodItem), "MMMM, y")}
                        </Text>
                      </TouchableOpacity>
                    </View>
                  );
