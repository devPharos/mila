...
const ClassesList = ({ periodClasses, group }) => {
  ...
  const animatedStyle = useAnimatedStyle(() => {
    return {
      maxHeight: showPeriodOptions
        ? withTiming(2000, { duration: 1000, easing: Easing.linear })
    ...
    <>
      <Container
        style={{
          ...
          marginBottom: 48,
          marginTop: 8,
          height: showPeriodOptions ? "auto" : 75,
          padding: 0,
        }}
      >
        <TouchableOpacity
        ...
        >
          ...
          <Feather
            style={{ color: theme.colors.secondary, fontSize: 22 }}
        ...
        <Animated.View style={[animatedStyle, styles.container]}>
          {periodClasses.map((periodClass, index) => {
            const icon =
              periodClass.presenceStatus === "Sick"
                ? theme.icons.Sick
                : periodClass.presenceStatus === "Absent"
                ? theme.icons.Absent
                : periodClass.presenceStatus === "Transfer"
                ? theme.icons.Transfer
                : periodClass.presenceStatus === "Vacation"
                ? theme.icons.Vacation
                : periodClass.presenceStatus === "Half Present"
                ? theme.icons.Halfpresent
                : theme.icons.Present;
            const periodColor =
              periodClass.presenceStatus === "Sick"
                ? theme.colors.Sick
                ...
                : periodClass.presenceStatus === "Vacation"
                ? theme.colors.Vacation
                : periodClass.presenceStatus === "Half Present"
                ? theme.colors.Halfpresent
              ...
              <View key={index} style={{ width: "100%" }}>
                <TouchableOpacity onPress={() => setClassInfo(periodClass)}>
                  <ClassesListItem
                    style={{
                      width: "100%",
                      borderLeftColor: periodClass.periodColor,
                    }}
                  >
                    <View style={{ paddingLeft: 16 }}>
                      ...
                      <Text
                        style={{ width: 80, color: "#868686", fontSize: 12 }}
                      >
                    ...
                    >
                      {periodClass.grades.length > 0 ? (
                        ...
                        >
                          ...
                          >
                            {capitalizeFirstLetter(periodClass.grades[0].name)}
                          </Text>
                          <Text style={{ color: theme.colors.Present }}>
                            {periodClass.grades[0].score}%
                          </Text>
                        </View>
                      ) : (
                        <>
                          <Text style={{ color: "#555", fontSize: 13 }}>
                            {periodClass.presenceStatus}
                          </Text>
                          {periodClass.icon}
                        </>
                      )}
